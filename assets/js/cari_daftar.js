...
L.Control.OSMGeocoder = L.Control.extend({
	options: {
		...
		callback: function (results) {
			...
			var bbox = results[0].boundingbox,
                ...
                x = (parseFloat(bbox[1]) + parseFloat(bbox[0]))/2;
                y = (parseFloat(bbox[2]) + parseFloat(bbox[3]))/2;
	...
	onAdd: function (map) {
		...
		var className = 'pencarian',
		container = this._container = L.DomUtil.create('div', className);
        header = L.DomUtil.create('div', 'ataspencarian');
        container.append(header);
        this._dalamatas = L.DomUtil.create('div', 'center');
		this._dalamatas.innerHTML = '<div style="color:white"><h6><b>Pencarian Lokasi</b></h6><div>'
		header.append(this._dalamatas);
	...
	_geocode : function (event) {
		...
		var params = {
			...
			q: this._input.value+' pekalongan',
